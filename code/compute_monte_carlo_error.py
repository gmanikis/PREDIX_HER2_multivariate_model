"""How much would re-running this analysis move each reported AUROC?
    ...
    python compute_monte_carlo_error.py [--results_dir <tree>]

Writes a table to stdout and, with --out, to an .xlsx. With no --results_dir it
locates the fold-level outputs itself; see `_default_results` for the rule.
...
def _default_results():
    """Locate the fold-level outputs, whichever layout this file is sitting in.

    Two layouts. In the DEPOSIT the artefacts are `results/` at the top level,
    one directory above this script; checking that first means a reviewer can
    run this straight out of the deposit with no arguments, which is the whole
    point of shipping it. In a WORKING TREE they sit in a dedicated
    analysis-output directory beside the scripts instead.

    Candidates are tested by looking for the pooled consensus evaluation itself
    rather than for a directory name, so an empty or half-copied tree is
    reported as missing instead of being silently accepted. Where a working
    tree offers more than one, the tree whose own run_provenance.json records
    the later generation time wins, and main() prints the directory it read so
    the choice is never invisible. Pass --results_dir to decide it explicitly.
    """
    here = Path(__file__).resolve().parent
    for cand in (ROOT / "results", here / "results"):
        if (cand / "global" / "global_consensus_eval.pkl").exists():
            return cand
    found = sorted({p.parents[1] for p in
                    ROOT.glob("*/results/global/global_consensus_eval.pkl")})
    if not found:
        return ROOT / "results"      # reported as missing, with the path shown
    return max(found, key=lambda r: (_generated_utc(r), str(r)))


def _generated_utc(results: Path) -> str:
    """A results tree's own record of when it was produced; "" if it has none."""
    try:
        import json
        return str(json.loads((results / "run_provenance.json").read_text(
            encoding="utf-8")).get("generated_utc", ""))
    except Exception:
        return ""
