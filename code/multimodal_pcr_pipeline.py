...
This pipeline supports both analyses explicitly:
  ...
  --univariate_screen none      (sensitivity analysis)
      Reproduces the submitted manuscript's behaviour, in which the univariate
...
PRIMARY ANALYSIS — elasticnet mode
  ...
  Stage B: Winner tuned over a manual grid evaluated INSIDE each inner fold and
           restricted to its own signature, so the tuned configuration is the
           configuration deployed.
           Outer refit on winner signature features + Platt calibration if needed.
           OOF scores via make_oof_signature (same model, same config).
...
TRAINING POPULATION (elasticnet mode only, --training_data)
  cc_only : every unimodal model is trained AND evaluated on the complete-case
            cohort — the same patients throughout, no one else entering at any
            stage.
  expanded: each unimodal model trains on ALL patients carrying that modality,
            minus the test patients. Outer test sets are still drawn from the
            complete-case cohort, so the modalities stay paired on one cohort.
...
# leakage-free analysis and False for the sensitivity analysis that reproduces
# the submitted manuscript's design, in which the univariate step had already
...
# ---------------------------------------------------------------------------
# CORRELATION HANDLING (read this before altering either set)
# ---------------------------------------------------------------------------
# TWO different things are governed here, by two separate constants so that
# neither can be switched by a change aimed at the other: the per-fold Tier 3
# correlation filter (CORR_FILTER_MODS), and the correlation-cluster pooling
# performed once at the consensus stage in _aggregate_signature
# (CONSENSUS_DEDUP_MODS). The first is off; the second is on.
#
# WHY TIER 3 IS OFF. Its job is done entirely by the fixed, outcome-blind
# TIER1_REMOVE list, which is extended far enough that no pair of candidate
# features exceeds |r| = 0.90 on this dataset (verified by preflight.py, which
# FAILS the run if any pair does). A per-fold filter keeps whichever cluster
# member wins that fold's univariate contest, so the surviving representative
# rotates between folds and a pair of near-identical features splits its
# selection frequency: DNA_coding_mutation_TP53 and its identical twin
# ..._TP53_oncokb would each carry a partial frequency summing to ~1 and BOTH
# reach the pooled consensus signature. Deciding redundancy once, a priori and
# on biology, avoids that artefact and removes a stage from the Methods.
#
...
# dataset it is a no-op.
# HOW THE CONSENSUS SIGNATURE IS AGGREGATED relative to the locked classifier.
# See finalize_consensus() for the full rationale.
#   "winner_folds"     : THE DEFAULT. Restrict the aggregation to the outer
...
#                        the warning below.
#   "all_folds"        : every fold contributes its own winner's signature,
#                        mixing families. Retained as a sensitivity setting so
#                        the choice can be isolated without editing code.
#
...
# therefore ranks and sizes K on a strictly larger set: K moves 7 -> 9 for
# global DNA, 10 -> 11 for global RNA and 4 -> 5 for T-DM1 DNA, and the locked
# signature can acquire features that outer preprocessing routinely drops
# (which then surface as "dropped by fold preprocessing" at refit and inflate
# n_model_features in the external table). That growth is an artefact of which
# object is aggregated, not the coherence the setting is meant to buy.
# winner_folds has no such mismatch. winner_all_folds is kept for sensitivity
# analysis and intersects with `candidate_features` to remove the artefact.
SIGNATURE_SOURCE      = "winner_folds"

CORR_FILTER_MODS      = set()             # Tier 3, per fold — off (see above)
...
# The winner classifier is then fully tuned in Stage B, over a manual grid run
# inside each inner fold and restricted to that winner's own signature.
STAGE_A_PARAMS = {
    "ElasticNet_LR": {"C": 0.1},
...
def build_classifiers(seed):
    ...
    The `seed` argument is accepted for call-signature compatibility and is
    deliberately NOT propagated into classifier constructors.
    """
    return {
        "ElasticNet_LR": {
...
# Per-scenario, within-modality, outcome-blind deduplication. Used only when
# --dedup_per_scenario is passed; see that flag's help and the module docstring.
from feature_deduplication import deduplicate, audit_frame  # noqa: E402
...
TIER1_REMOVE = [
    ...
    # mutational burden at different granularities, so the whole cluster is
    # named here and the rule stays correct for a data release that carries
    # any of them. On the canonical file
    # (clin_multiomics_curated_metrics_PREDIX_HER2_new.txt, 197 x 114) only
    # DNA_TMB_clone is present, so it is the only removal this block effects;
    # DNA_TMB_uniform and DNA_pTMB are no-ops here. The removal that does take
    # effect is exact: DNA_TMB_clone duplicates DNA_TMB_clone_oncogenic
    ...
    # ==================================================================
    # Everything below closes the gap that a per-fold Tier 3 correlation
    # filter would otherwise have to cover. Tier 3 is off (see
    # CORR_FILTER_MODS), so this list must leave NO pair of candidates
    # above |r| = 0.90.
    ...
    # nothing in this cohort. Retaining both gives each half the selection
    # frequency of one real signal and puts both into the consensus
    ...
    # The immune block above keeps TILs, mRNA-CD8A and NK-cells for their
    # distinct biology, but the three still cluster at r = 0.913-0.942. A
    # per-fold Tier 3 filter would prune two of them anyway, silently and
    # differently in each fold — a contradiction between two stages. The
    # decision is therefore made here, once, and Tier 3 stays off.
    #
    ...
    #
    # RNA_NK-cells is RETAINED. It correlates with TILs at 0.913, but once
    ...
    # over-pruning and would discard the innate-immunity signal the immune
    # block above is designed to keep.
    # ------------------------------------------------------------------
    ...
    #
    # A correlation filter restricted to RNA and DNA on the ground that
    # "Clin, Prot and WSI have <= 5 features" leaves this pair untouched:
    # Prot carries 19 features, not 5. Undeduplicated, Prot_ERBB2 and
    # Prot_HER2_amplicon both reach the pooled consensus signature, and the
    # DHP signature again, although they correlate at 0.923.
    #
    ...
    # ------------------------------------------------------------------
    # RNA: FCGR3B — excluded on measurement-validity grounds.
    #
    ...
    #
    # IMPACT DISCLOSURE (not a justification; recorded so the exclusion is
    # not silent). FCGR3B is not an inert feature on this cohort: retained,
    # it enters the T-DM1 arm signature and the pooled transcriptomic model,
    # so excluding it moves the T-DM1 arm and both external validations. The
    # exclusion is decided on the measurement grounds above, ahead of any of
    # that; the resulting numbers are reported as they fall, in either
    # direction.
    #
...
def remove_near_zero_variance(
    ...
    for col in X_train.columns:
        ...
        # OBSERVED values. Ungated, the ratio rule alone would remove any binary
...
def remove_high_correlation(
    ...
    about magnitude of signal, not direction). A variance-based keeper is
    deliberately not used, for three reasons:
...
# No helper maps complete-case split indices onto the expanded frame here, and
# none is needed: Stage B tunes over the in-fold, signature-restricted manual
# grid in _fit_signature_model rather than over a GridSearchCV across the
# outer-preprocessed matrix, so the split indices never have to be translated.
...
def _check_calibration(y_true_list, y_pred_list,
    ...
    DIAGNOSTIC ONLY. Calibration is always applied (see
    `_apply_global_calibration` below), so this function exists for its
    slope/Brier diagnostics in the terminal log; its return values are
    recorded in `fold_dict["calibration"]` for transparency and gate
    nothing.
...
def make_oof_signature(clf_name, best_params, signature_feats,
    ...
    gives the sigmoid the maximum training data (~100 patients, against the
    ~30 a nested cv=3 sigmoid would see), and ensures all modality OOF columns
...
def _fit_signature_model(
    ...
    - Stage B tunes the winner on its SIGNATURE subset, so the tuned
      configuration is the configuration deployed. Tuning it instead by
      GridSearchCV over the full outer-preprocessed matrix would score
      inner-validation rows through outer-fitted preprocessing — internally
      optimistic, though never outer-test-leaking — which is why it is not
      done that way.
    ...
    #              purely for REPORTING. Ranks and magnitudes are different
    #              quantities, so they are accumulated and labelled apart:
    #              storing ranks alone while the Methods and the consensus
    #              summary speak of mean |SHAP| importance would report one
    ...
    mean_aurocs_A = mean_pruned_auroc   # pruned-signature AUROCs, not all-feature

    # ── STAGE B: winner hyperparameter tuning — in-fold and signature-true ───
    # Stage B does NOT use GridSearchCV over the outer-preprocessed matrix
    # (X_tr_p). That design carries two inconsistencies:
    #   (1) inner-validation rows would be scored through preprocessing —
    #       including the Tier 2.5 univariate outcome screen — fitted on ALL
    #       outer-training patients, inner-validation rows included, so
    #       inner_cv_auroc_B would be optimistic and hyperparameter choice
    #       mildly biased toward screen-favoured configurations (no
    #       outer-test leakage, but inconsistent with Stage A's protocol);
    #   (2) it would tune on the FULL feature matrix while the deployed model
    #       is refit on the winner signature — tuning a different model than
    #       the one shipped.
    ...
    # also cheaper than a full-matrix GridSearchCV.
    #
...
# ==============================================================================
# SECTION 4b — SUPPLEMENTARY MODEL LOGIC (SHAP, INNER CV, OOF, FUSION)
# ==============================================================================
# Used by the supplementary modes (best_per_fold, ensemble_weighted), not by
# the primary elasticnet mode.
...
def inner_cv_all(X_train, y_train, inner_splits, active_clfs, inner_jobs=1):
    ...
    SUPPLEMENTARY — used only by the non-primary modes (best_per_fold,
...
def run_experiment(df_cc_exp, features, clin_key, splits, exp_name,
    ...
    # prevent BLAS oversubscription. Assigning os.environ[...] inside each
    # worker instead would run too late — BLAS pools are initialised at numpy
    # import time.
    #
    ...
    print(f"  Univariate screen: "
          + (f"IN-FOLD (BH q<={UNIV_SCREEN_FDR_Q}, max_k={UNIV_SCREEN_MAX_K}, "
             ...
             "DISABLED (candidate pool was outcome-informed upstream)"))
...
def _fit_elasticnet(X_tr_p, y_tr, X_te_p, y_te, inner_splits, fcols,
                    ...
                    # Fallback path (not reached by the primary pipeline, which
                    # always supplies mod_full_df — the expanded modality pool
                    # under --training_data expanded, the complete-case frame
                    # under cc_only):
                    X_raw_df=None,
                    inner_jobs=1):
    """
    ...
    # OOF generation — from the supplied modality frame (primary) or, if none
    # was supplied, from the complete-case matrix alone
    if mod_full_df is not None:
        ...
        # Fallback: OOF from the complete-case matrix, for a caller that
        # supplies no modality frame at all
...
def _aggregate_signature(folds, size_strategy="median",
    ...
    """
    # clf_key selects WHOSE per-fold signature is aggregated.
    #   None  : each fold contributes its own winner's signature — the rule
    #           when SIGNATURE_SOURCE == "all_folds".
    ...
    if df_cc is not None and feat_cols is not None and len(all_feats) > 1:
        ...
        # BINARY FEATURES MUST STAY ELIGIBLE. Requiring nunique() > 2 here would
        # silently exempt every binary feature from consensus deduplication, and
        # the duplicated pairs in this panel are precisely binary mutation
        # indicators (DNA_coding_mutation_TP53 and ..._TP53_oncokb are identical
        # on all 190 shared rows, likewise the GATA3 pair). The one class of
        # feature that most needs deduplicating would then be the one class
        # exempt from it, and both TP53 columns would reach the pooled AND the
        # DHP consensus signature as if they were two independent findings.
        #
        ...
        if len(cont) > 1:
            ...
            for cluster in clusters:
                ...
                # enter that fold's signature. Summing member counts would let
                # each such fold contribute 2 — selection_frequency could then
                # exceed 1.0 in the consensus summary — and would inflate
                # pool_imp_sum, the ranking key, exactly for co-selecting
                # clusters. Count each fold once; credit the fold's best
                # member rank.
                #
                # THE SIGNATURE SOURCE MUST MATCH THE ACCUMULATION. Reading
                # winner_signature and winner_clf unconditionally here would
                # mix two classifiers inside one ranking whenever
                # SIGNATURE_SOURCE == "winner_all_folds", because the
                # accumulation above keys on signatures_all[clf_key] and
                # inner_importance[clf_key]: the representative's pooled score
                # would come from whichever family won each fold while every
                # other feature's score came from the locked classifier.
                ...
                # cluster fires. Guarded anyway: it would surface silently on
                # the next dataset, and the only evidence it had fired would be
                # a [CONSENSUS-POOL] log line.
...
def finalize_consensus(results, ALL_MODS=("Clin", "RNA", "DNA", "Prot", "WSI"),
    ...
    for mod in ALL_MODS:
        ...
        # CONSENSUS_DEDUP_MODS, NOT by CORR_FILTER_MODS: the per-fold Tier 3
        # filter is off, while this consensus-stage safety net is on.
        ...
        # CLASSIFIER FIRST, THEN THE SIGNATURE THAT CLASSIFIER CHOSE.
        #
        # The two must be aggregated over the SAME fold set. Aggregating the
        # classifier as the modal winner while pooling the signature over every
        # fold regardless of which family won it would let a fold won by
        # SVM_Linear contribute features to a signature reported alongside
        # ExtraTrees. The deliverable would be "the modal classifier" plus "the
        # features the fold winners collectively chose" — not one object, and a
        # paragraph to explain.
        #
        # The signature aggregation is therefore restricted to the folds the
        # locked classifier actually won, so the pair is coherent by
        # construction and the Methods sentence is one line: the modal winning
        # classifier, and the features that classifier selected.
        #
        ...
        # so swapping it in wholesale would discard the consensus-stage dedup
        # that keeps duplicated features out of the signature. Restricting the
        # folds keeps the dedup, the K rule and the importance weighting
        # untouched.
        #
...
def _refit_consensus_unimodal_fold(
    ...
    # Which regime are we in? The caller knows, and says so explicitly rather
    # than leaving it to be inferred here.
    #
    # Inferring it as `len(X_tr_raw_df) != len(cc_df_full)` would be wrong
    # whenever the expanded pool happens to have the same
...
def parse_args():
    ...
    p.add_argument("--nzv_freq_arm",    type=float, default=0.98,
        ...
        # Python 3.13+ — before any argument is parsed, so the script would
        # not start at all.
    ...
    # ── Coherence of the locked (classifier, signature) pair ─────────────────
    p.add_argument("--signature_source",
        ...
        help="Whose per-fold signature is aggregated into the locked "
             "signature. winner_folds (DEFAULT): restrict to the outer "
             ...
             "all_folds: every fold contributes its own winner's signature, so "
             "a fold won by SVM_Linear feeds a signature reported alongside "
             "ExtraTrees. All three keep the correlation "
             "dedup and the K rule.")

    # ── Skip discovery and re-finalise from an existing PKL ──────────────────
    ...
    # Experiments
    p.add_argument("--dedup_per_scenario", action="store_true",
                   help="Remove within-modality redundancy inside EACH "
                        "scenario's own cohort, once, before any split. "
                        "Outcome-blind: correlation, exact agreement and "
                        "Cohen's kappa are computed between features only, so "
                        "this runs before splitting without leaking the "
                        "outcome. Off by default, so omitting it reproduces "
                        "the previous locked run exactly. See "
                        "feature_deduplication.py for the rule and why one "
                        "statistic cannot serve all three feature types.")
...
def write_cv_design_statement(args, results_dir):
    ...
    outer evaluations)". The code implements the latter and nothing else
    (RepeatedStratifiedKFold outer, StratifiedKFold inner), so the
    shuffle-split wording describes no analysis this pipeline performs. Every
    manuscript passage that describes the CV design must be copied from this
    file, so the text cannot drift from the code. The same content is embedded
    in run_provenance.json under "cv_design".
    """
...
def main():
    ...
    if args.data_path is None or not Path(args.data_path).exists():
        raise SystemExit(
            "==============================================================================\n"
            "PREDIX HER2 - NO-PATIENT-DATA VARIANT OF THE DEPOSIT\n"
            "==============================================================================\n"
            "multimodal_pcr_pipeline.py (model training)\n"
    ...
    print(f"  Univar. screen : {args.univariate_screen}"
          + ("  ← leakage-free primary analysis"
             if UNIVARIATE_SCREEN else
             "  ← candidate pool was outcome-informed upstream"))
    ...
    for exp_name in args.experiments:
        ...
        # ── Per-scenario deduplication, before any split ───────────────────
        # Collinearity is a property of a population: two mutation columns are
        # r = 1.000 within the DHP arm and only borderline pooled, and the 17q
        # passengers MED1 and RPL19 carry the SAME segment value as ERBB2 in
        # 85-88% of patients while correlating at only 0.69-0.83. So the rule
        # runs inside each scenario's own cohort.
        #
        # Computed on df_cc_exp, the complete-case cohort, in BOTH training
        # modes. That is deliberate: it gives cc_only and expanded an identical
        # feature pool, so a difference between them is attributable to the
        # training population and not to two pools that were never the same.
        #
        # Within modality only. Fusion is at the level of predictions, so an RNA
        # feature and a DNA feature never share a design matrix and their
        # correlation is not collinearity for any fitted model.
        features_exp = features
        if args.dedup_per_scenario:
            _cols = sorted({c for k, v in features.items() for c in v})
            _res = deduplicate(df_cc_exp, _cols, scenario=exp_name)
            _dropped = set(_res.drop)
            features_exp = {k: [c for c in v if c not in _dropped]
                            for k, v in features.items()}
            _aud = audit_frame(_res)
            _aud.to_csv(exp_dir / f"{exp_name}_deduplication_audit.csv",
                        index=False)
            print(f"\n[DEDUP:{exp_name.upper()}] removed {len(_res.drop)} of "
                  f"{len(_cols)} candidates within modality "
                  f"(n={len(df_cc_exp)})")
            for _d in _res.decisions:
                print(f"    {_d.dropped:34} -> kept {_d.kept:30} "
                      f"{_d.statistic}={_d.value:.3f}")
            if _res.drop:
                print(f"    audit written to "
                      f"{exp_name}/{exp_name}_deduplication_audit.csv")
        ...
        # generated from args — if a splits file carrying no metadata block
        # delivered a different design, they would certify a CV design the
        # run did not execute. Files that carry the metadata block cannot
        # reach here mismatched.
        if len(splits) != n_outer * n_rep:
            print(f"[CV-DESIGN] WARNING: {exp_name} runs {len(splits)} outer "
                  f"folds from a splits file with no metadata block, but args imply "
        ...
        if args.consensus_only:
            # Re-derive the consensus from an existing discovery PKL without
            # repeating the per-fold model search. Only valid when the PKL was
            # produced with the same candidate pool — any change to
            # TIER1_REMOVE invalidates it — so this path is for isolating
            # --signature_source, not for shipping.
            ...
            for mode in modes:
                ...
                run_experiment(
                    df_cc_exp=df_cc_exp, features=features_exp, clin_key=clin_key,
