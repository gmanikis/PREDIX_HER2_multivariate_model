...
3. FEATURES ARE STANDARDISED WITHIN EACH COHORT. The three cohorts are on
   ...
   cohort before the model is applied, by the per-cohort z-score: mean 0, SD 1.
...
NOTE that this particular alias is INERT: PREDIX carries RNA_ADC_trafficking
under neither spelling. `harmonise_columns` therefore applies an alias only when
the target exists in PREDIX, and reports any it declined to apply. Renaming
unconditionally would instead create a dead column in the external frame and
write a "renamed / rescued" row into Feature_provenance for a feature that is
in no model — a claim a supplementary table must not make.
...
-------
  ...
  WHY THE SUFFIX EXISTS. A sensitivity run writing into its own directory but
  keeping identical BASENAMES produces two files called
  revfig06_external_validation.pdf that differ only by the directory above them,
  and opening the obvious path then gives no clue which model is being read.
  Pass a suffix — --output_suffix _POOLED for a pooled run, say — so any second
  external-validation run is distinguishable by filename alone.
"""
...
# Warnings are filtered by category rather than globally: a blanket ignore would
# silence cv_estimands' two guard warnings — that a PKL carries only positional
# test_idx rather than test_pids, and that N of R repeats do not predict every
# patient. The second matters here: the locked-mode internal comparator is read
# straight from the consensus PKL, so an aborted pipeline run would yield
# per-repeat AUROCs computed on subsets of patients, averaged into
# internal_AUROC, with no trace in the log or the workbook — and internal_AUROC
# is the number the external result is judged against. Keep RuntimeWarning
# audible.
...
# Appended to output basenames; set from --output_suffix in main().
OUTPUT_SUFFIX = ""

# The cross-platform harmonisation: features are standardised within each
# cohort by z-score. It is a constant rather than a command-line option.
HARMONISATION = "zscore"
...
def harmonise_columns(df_ext, cohort_name, predix_cols=None):
    ...
    A rename is applied only when the TARGET EXISTS IN PREDIX. The alias map
    carries RNA_ADC_traficking -> RNA_ADC_trafficking, and the production PREDIX
    file contains that feature under NEITHER spelling. Renaming unconditionally
    would therefore create a dead column in the external frame; and because
    `shared_features` iterates PREDIX columns, the feature would be invisible to
    `excluded` while a "renamed / rescued" row was still written into
    Feature_provenance — a row in a supplementary table for a feature that is in
    no model. Guarding on the target keeps the table truthful.
    """
    renames, skipped = {}, {}
    ...
    for old, new in skipped.items():
        print(f"  [{cohort_name}] alias {old!r} -> {new!r} NOT applied: "
              f"{new!r} is absent from PREDIX. The "
...
def shared_features(df_predix, df_ext, cohort_name):
    ...
    for col in predix_rna:
        ...
        if ext_vals.notna().sum() == 0:
            ...
            continue
        # Qualifying on "at least one observed value" would let a feature with
        ...
        # constant externally: SD 0 -> all zeros. Neither would be reported.
        # The threshold is empirically slack on the current two cohorts (zero
        # missing, zero near-constant among shared features) — it guards the
        # next one.
...
def standardise_within_cohort(X, method="zscore"):
    ...
    zscore : subtract the cohort mean, divide by the cohort SD. Preserves the
             ...
             scale differences. It is the only scheme this script implements.

    The `method` parameter is deliberately KEPT — several call sites pass it
    positionally, so dropping it would silently change this function's arity —
    but "zscore" is its only accepted value and anything else raises rather
    than quietly standardising by some other rule.

    It is computed WITHIN each cohort independently and uses no outcome
    information, so it does not leak the external labels into the model.
    ...
    if method != "zscore":
        raise ValueError(
            f"unknown standardisation method: {method!r}. 'zscore' is the only "
            "supported harmonisation.")
    ...
    for j in range(X.shape[1]):
        ...
        if obs.sum() < 2:
            continue
        # Compute on the finite subset, not with nanmean/nanstd: those ignore
        # NaN but NOT +/-inf, while `obs` (np.isfinite) excludes both. One
        # infinite value would otherwise make mu and sd infinite, every z-score
        # NaN, and nan_to_num would later zero the whole feature — silently
        # dropping it from the model while it still counted in n_model_features.
        mu = np.mean(col[obs])
        sd = np.std(col[obs])
        if sd > 1e-12:
...
def build_model(name, params=None):
    ...
    if name == "ElasticNet_LR":
        ...
        # SVM_RBF is present so that this registry covers all six families in
        # the pipeline's registry. Were it missing and an RNA consensus winner
        # came back as SVM_RBF, the locked run would die with "unknown
        # classifier", and the only way forward would be substituting a
        # different classifier — i.e. re-tuning the locked model. Production
        # passes five families so it cannot win today; the two lists are coupled
        # by convention only, which is why the mismatch is worth closing here.
...
def internal_cv_estimate(X, y, clf_name, grid, n_splits=5, n_repeats=10,
    ...
    (cv_estimands). This function serves the sensitivity path only.
    """
...
def load_locked_consensus(results_dir, exp_name, predix_path):
    ...
    if prov.exists():
        ...
        if not trained_sha:
            # A missing hash is an error, not a reason to skip the check. The
            # identity check downstream compares SETS OF ROW POSITIONS, so its
            # entire guarantee that the refit cohort is the pipeline's cohort
            # rests on this hash. A different file with the same row count and
            # the same complete-case positions would otherwise pass unnoticed.
            raise ValueError(
...
def validate_cohort(df_predix, df_ext, cohort_name, method, clf_name,
                    ...
                    locked_spec=None):
    """
    Run the full lock-and-apply validation for one cohort, with features
    standardised within each cohort by z-score.
    ...
    if arm_code is not None:
        # The missing-column case is raised, not folded into a compound
        # condition it could fall through silently: doing so would refit on all
        # 197 patients while every output still claimed arm matching. Locked
        # mode would catch that via the identity check; grid mode would not.
    ...
    # Full n_boot, not a reduced count: the workbook note states that every CI
    # uses the same number of resamples, and the calibration SLOPE CI is one of
    # the intervals conclusions rest on, so it must not be the one computed at a
    # fraction of the stated resolution. The cost is seconds.
    ...
    return {
        ...
        "label": info["label"],
        "arm": info["arm"],
        # Record what the model was ACTUALLY refit on, as its own field.
        # `arm` is a property of the COHORT (which PREDIX arm it resembles) and
        # is constant, so reporting it as `matched_PREDIX_arm` would be true for
        # an arm-matched run and FALSE for a pooled one, where arm_code is None
        # and the refit uses every PREDIX patient carrying RNA. Deriving the
        # label from arm_code is what stops a pooled workbook and figure from
        # claiming arm matching that did not happen.
        ...
        # In LOCKED mode the internal comparator is read from the consensus PKL
        # and is NOT recomputed by this script; it comes from the pipeline's own
        # within-fold standardisation. Saying so explicitly is what stops
        # AUROC_drop_internal_to_external being read as a like-for-like
        # difference in harmonisation.
        "internal_harmonisation": ("pipeline within-fold standardisation "
                                   "(z-score); taken from the locked run, not "
                                   "recomputed here"
...
def build_report(results, td, fd):
    ...
    for r in results:
        ...
        rows.append({
            ...
            # always meant) so downstream consumers do not break; it is
            # always "zscore", and the internal_harmonisation column below
            # says what the internal comparator was standardised by.
            ...
            # Render "not estimable" rather than "nan–nan" when the
            # recalibration fit did not converge, and carry the intercept's
            # interval through: it is computed, so it belongs in the table.
    ...
    # rest did not.
    # One row per cohort, so every result contributes its provenance exactly
    # once.
    prov = []
    for r in results:
        if r is None:
    ...
    _write_sheet(
        ...
        note=("INDEPENDENT EXTERNAL VALIDATION OF THE TRANSCRIPTOMIC pCR MODEL.\n"
              ...
              "z-score standardisation IS estimated on the external "
              ...
              +
              "HARMONISATION. The cohorts are on incompatible measurement scales, so "
              "features are standardised by the per-cohort z-score (mean 0, SD 1), "
              "computed within each cohort independently and without reference to "
              "outcome; the harmonisation column records it on every row.\n"
              # Interpolate the RESOLVED count, not the module constant:
              # otherwise a run with --n_boot 200 writes "2000 resamples" into
              ...
              "performance, standardised within training folds; it is read "
              "from the locked run and is NOT recomputed by this script, so "
              "AUROC_drop_internal_to_external compares an externally "
              "z-scored number against that fixed internal one.\n"
    ...
    _write_sheet(
        ...
        note=("Exactly which RNA features transferred to each external cohort, "
              ...
              "is absent from PREDIX is reported in the run log and produces "
    ...
    for ci, cohort in enumerate(cohorts):
        # Top row: internal vs external AUROC. One z-score series per cohort.
        ax = axes[0][ci]
        labels, vals, los, his, cols = [], [], [], [], []
        r = next((x for x in valid if x["cohort"] == cohort), None)
        if r is not None:
            labels.append("PREDIX internal")
        ...
        # Take the scope from arm_matched: never assert arm matching on a
        # pooled run.
    ...
    fig.suptitle(
        ...
        "Model locked on PREDIX and applied unchanged; features z-scored within "
        "each cohort",
...
def parse_args():
    ...
    p.add_argument("--output_suffix", default="",
                   help="Appended to the output basenames, e.g. '_POOLED'. Use "
                        ...
                        "file for why the suffix exists.")
    ...
    # There is no --methods flag: harmonisation is the per-cohort z-score, so
    ...
    p.add_argument("--locked_experiment", choices=("arm", "global"), default="arm",
                   help="Which experiment inside the locked results dir to "
                        "freeze. 'arm' (the default) "
                        ...
                        "signature retains the HER2DX HER2-amplicon score, so the two "
                        "can transfer differently to the same external cohort. "
...
def main():
    ...
    if args.predix is None or not Path(args.predix).exists():
        raise SystemExit(
            "==============================================================================\n"
    ...
    print(f"  Harmonising : {HARMONISATION} (per-cohort, outcome-blind)")
    ...
    # --locked_experiment global freezes the POOLED model instead of the
    # arm-matched one, and necessarily refits it on all PREDIX patients — that
    # is what makes it the pooled model. The incompatibility below therefore
    ...
    for name, df_ext in sources:
        ...
        # One row per cohort; features are harmonised by the per-cohort
        # z-score.
        results.append(validate_cohort(
            df_predix, df_ext, name, HARMONISATION, args.classifier,
