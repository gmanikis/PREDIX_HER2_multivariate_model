...
--------------------------------------
1. LOCK, THEN APPLY. The model is fitted once on PREDIX and frozen. Nothing is
   refitted on external data. Recalibration, where reported, is reported as a
   separate row so its effect is visible rather than absorbed.

2. THE COMPARISON IS LIKE FOR LIKE. The internal estimate quoted alongside each
   external result is recomputed on the SAME restricted feature set that the
   external cohort supports (44 of PREDIX's 48 RNA features). Comparing a
   48-feature internal AUROC against a 44-feature external AUROC would confound
   cohort transfer with feature availability.
...
(one f); PREDIX spells it `RNA_ADC_trafficking` (two f). An exact-name
intersection silently drops it — the very signature that defines the S3 group
in the treatment-selection scheme. FEATURE_ALIASES below repairs this and any
comparable mismatch, and the script prints what it renamed.
...
OUTPUTS
-------
  {out_dir}/tables/revision/external_validation.xlsx
  {out_dir}/figures/revision/revfig06_external_validation.pdf
"""
...
def harmonise_columns(df_ext, cohort_name):
    """Apply the alias map and report every rename, so nothing is silent."""
    renames = {c: FEATURE_ALIASES[c] for c in df_ext.columns
...
def load_locked_consensus(results_dir, exp_name, predix_path):
    ...
    if prov.exists():
        ...
        if trained_sha and trained_sha != here_sha:
            raise ValueError(
...
def validate_cohort(df_predix, df_ext, cohort_name, method, clf_name,
    ...
    df_ext, renames = harmonise_columns(df_ext, cohort_name)
    ...
    dp = df_predix.copy()
    if arm_code is not None and "Clin_Arm" in dp.columns:
    ...
    ext_cal = calibration_metrics(y_ext, p_ext, n_boot=min(n_boot, 500))
...
def build_report(results, td, fd):
    ...
    for r in results:
        ...
        rows.append({
            ...
            "calibration_slope_CI": (f"{cal['slope_ci'][0]:.2f}–"
                                     f"{cal['slope_ci'][1]:.2f}"),
    ...
    prov = []
    for r in results:
        if r is None or r["method"] != "zscore":
            continue   # identical across harmonisation schemes
        for f in r["features"]:
    ...
    _write_sheet(
        ...
        note=("INDEPENDENT EXTERNAL VALIDATION OF THE TRANSCRIPTOMIC pCR MODEL.\n"
              "The model is fitted on PREDIX and then FROZEN. No external patient "
              "contributes to feature selection, hyper-parameter choice, or any "
              "threshold. Nothing is refitted on external data.\n"
              ...
              "Each external cohort is predicted by the ARM-MATCHED PREDIX model: "
              "I-SPY2 (trastuzumab/pertuzumab + chemotherapy) by the DHP model, "
              "NCT02326974 (T-DM1 + pertuzumab) by the T-DM1 model.\n"
              ...
              f"intervals ({N_BOOT} resamples); the locked-mode internal CI is "
              "the patient-level CLUSTER bootstrap described above.\n"
    ...
    _write_sheet(
        ...
        note=("Exactly which RNA features transferred to each external cohort, and the "
              "reason for every exclusion.\n"
              "NOTE the rename: the external files spell the ADC-trafficking signature "
              "'RNA_ADC_traficking' with a single f, while PREDIX uses "
              "'RNA_ADC_trafficking'. An exact-name join silently drops it — and it is "
              "the signature defining the S3 group of the treatment-selection scheme. "
              "The rename is applied explicitly and recorded here."))
    _write_sheet(
    ...
    path = td / "external_validation.xlsx"
    wb.save(path)
    print(f"\n  -> {path.name}")
    ...
    for ci, cohort in enumerate(cohorts):
        ...
        ax.set_title(f"{cohort} — matched to PREDIX {r0['arm']} arm\n"
    ...
    plt.tight_layout()
    _savefig(fig, fd / "revfig06_external_validation.pdf")
    return df
...
def main():
    ...
    for name, df_ext in sources:
        dfx, _ = harmonise_columns(df_ext, name)
    ...
    if args.pooled_arms and any(locked_dirs.values()):
        raise SystemExit("--pooled_arms is incompatible with --locked_*: the "
                         "locked models are arm-specific by construction.")
    locked = {}
    for name, _ in sources:
        if locked_dirs.get(name):
            exp = "dhp" if COHORTS[name]["arm_code"] == 0 else "tdm1"
            locked[name] = load_locked_consensus(
                locked_dirs[name], exp, args.predix)

    results = []
    for name, df_ext in sources:
        arm_code = None if args.pooled_arms else COHORTS[name]["arm_code"]
