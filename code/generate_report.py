...
_CE_CACHE = {}
_N_BOOT = 2000
# Seed base and tag scheme are IDENTICAL to revision_analyses.py (BOOT_SEED +
# crc32(tag) % 10000), so the same cell — e.g. Global/consensus/Clin AUROC —
# prints the same CI in fig01, PREDIX_HER2_results.xlsx and
# revision_performance_CI.xlsx. One number per quantity, everywhere.
_BOOT_SEED_BASE = 20240517


def _shared_seed(tag):
    import zlib
    return _BOOT_SEED_BASE + zlib.crc32(tag.encode()) % 10000
...
def build_performance_ci_table(data, td):
    ...
    for name, _ in METRICS:
        if name in REPEAT_METRICS:
            ...
            hdr_row1 += [f"{name} mean over folds at fold thresholds "
                         f"[95% cluster CI]", "", "", ""]
