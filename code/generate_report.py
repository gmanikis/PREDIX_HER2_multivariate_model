...
def _stability_thresh(results_dir=None):
    ...
    pipeline's own argparse defaults. A hand-typed dict here could not track an
    overridden threshold, and these numbers feed both the Tier-4 rows and the
    Pruning_Statistics stable-feature columns of a shipped supplementary table.
    """
...
def _tier1_remove_from_pipeline():
    ...
    A hand-copied list in the pruning-report sheet goes stale the moment the
    pipeline's own list changes, and nothing errors when it does — a wrong count
    simply ships in a supplementary table. Parsing it with ast gives the value
    one home, and an unparseable pipeline raises here rather than emitting a
    wrong figure.
    """
    ...
    # Honour the conditional form, `if not KEEP_RNA_FCGR3B: TIER1_REMOVE.append(...)`.
    for node in ast.walk(tree):
...
def detect_mode(data):
    ...
    Both modes route through _fit_signature_model and produce the same
...
def get_signature_feats(fold):
    ...
    Both expanded and cc_only use winner_signature (from _fit_signature_model).
    Falls back to selected_features for any PKL that does not carry it.
    """
...
_CE_CACHE = {}
# These are imported from cv_estimands, which BOTH this script and
# revision_analyses.py depend on, so they cannot drift apart. Hard-coding
# n_boot and the seed base here while revision_analyses rebinds both from its
# CLI would let `revision_analyses.py --seed 12345 --n_boot 5000` produce two
# workbooks carrying DIFFERENT intervals for the same quantity, with nothing to
# indicate why — agreement at the defaults, and one flag to break it.
...
def _mean_signed_shap(sh):
    ...
    positive-class direction. Both call sites go through here.
    """
    sv = sh["shap_values"]
...
def fig_consensus_signatures_main(data, fd):
    ...
    for ri, mod in enumerate(UNIMODALS):
        ...
        for ci, sc in enumerate(scenarios):
            ...
            # measured exactly on the consensus PKLs: 3.000000 for K=5, 2.500000
            # for K=4, 2.000000 for K=3. The value is a normalised rank in (0, 1],
            ...
            # downstream consumers. Renaming it would break them, so it is the
            # user-facing label that carries the accurate name.
    ...
    for sc in scenarios:
        ...
        for ci, mod in enumerate(UNIMODALS):
            ...
            ax.set_title(f"{mod}  (n_sig = {len(consensus_sig)})",
                          color=col, fontsize=11, fontweight="bold")
        sc_lbl = "Global Model" if sc == "Global" else f"{sc} Arm"
        # Say plainly that the classifier is not fixed across the cloud.
...
def build_excel(data, td):
    ...
    if not any_pooled:
        ws6.cell(row=2, column=1, value=(
            "Pooled metrics unavailable: no results['_pooled_metrics'] in the "
            "PKL. Run multimodal_pcr_pipeline.py to populate this sheet."))
    ...
    if not any_cf:
        ws7.cell(row=2, column=1, value=(
            "No cross_arm_preds found in PKL. Run multimodal_pcr_pipeline.py "
            "with cross-arm prediction enabled to populate this sheet."))
...
def build_performance_ci_table(data, td):
    ...
    #
    # READ THIS BEFORE QUOTING Sensitivity OR Specificity.
...
def build_feature_pruning_report(data, td, results_dir=None):
    ...
    # True stage order, stated once so a reader is not left wondering about the
    # numbering gap between Tier 2 and Tier 4.
    ...
    # Read the real constant and fail loudly if the pipeline stops defining it.
    # A guard of the form `FUSION_C_GRID if 'FUSION_C_GRID' in dir() else
    # "<hand-typed list>"` does NOT work here: inside a function `dir()` lists
    # LOCALS, so the guard is always False and the hand-typed fallback always
    # wins — silently, and correctly right up until the constant changes, which
    ...
    r = _row(r, "Diagnostic slope (does NOT gate anything)",
             ...
             "NOT read by the calibration path — it gates nothing. Platt scaling is "
             "applied unconditionally, on every fold that can fit a sigmoid, so "
             "`needs_platt` must not be read as a switch that decides whether a fold "
             "was calibrated.", italic=True)
    ...
    if not any_pooled:
        ws.cell(row=2, column=1,
                value="No _pooled_metrics found in any PKL. "
                      "Run multimodal_pcr_pipeline.py with pooled metrics enabled.")
...
def build_consensus_sheet(data, td):
    ...
    # 1/K..K/K and sum to (K+1)/2 (measured exactly on the consensus PKLs: 3.0
    # for K=5, 2.0 for K=3). Labelling it "Mean |SHAP importance|" would claim a
    # unit the column does not have, in the workbook the manuscript quotes. The data
