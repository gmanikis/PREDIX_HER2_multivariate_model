...
perform. Each has a section below and a corresponding output artefact.
  ...
  1. UNCERTAINTY IS UNDERSTATED.
     ...
     → ESTIMAND (cv_estimands.py): the metric is computed on each CV repeat's
       ...
       before computing AUROC is an inviting alternative and is NOT used
  ...
  5. THE TREATMENT-SELECTION SCHEME WAS PRESENTED AS A DECISION TOOL.
     → WITHDRAWN from the revision rather than repaired. The S1/S2/S3 scheme
       ...
       to 0.25. This module implements no S1/S2/S3 analysis and the claim is
       not made in the revised manuscript.
...
-------
  ...
  (revfig06_external_validation.pdf is drawn by make_revfig06_external_validation.py
   from the external-validation workbook, after apply_locked_external_validation.py)
...
# Warnings are filtered by category, never with a bare filterwarnings("ignore")
# installed ahead of `import cv_estimands` below: that would silence the same
# module's RuntimeWarning that "N of R repeats do not predict every patient" —
# the one signal that a partially-completed or aborted discovery run is being
# summarised. CIs would then be computed on incomplete repeat matrices with
# nothing in the log. Silence only the noisy library-deprecation categories;
# keep RuntimeWarning and UserWarning audible.
...
# Single definition in cv_estimands, shared with generate_report.py so the two
# cannot drift. --n_boot / --seed still override both, in main().
...
#
# Averaging each patient's R probabilities into one number first is NOT the
# same thing and is deliberately not done: it scores a 200-model ensemble, and
# for a near-constant model it is dominated by the held-out-outcome artefact (a
...
def pool_oof_by_patient(folds, pred_key="y_pred", label_key="y_test",
    ...
    if key is None:
        raise KeyError(
            ...
            "Run multimodal_pcr_pipeline.py, "
...
def calibration_metrics(y, p, n_bins=N_CAL_BINS, n_boot=None, seed=None):
    ...
    slope, intercept
        ...
        THE TWO INTERCEPTS ARE NOT THE SAME QUANTITY, and conflating them is an
        easy mistake. The intercept `a` from this two-parameter fit is NOT
        ...
        does not measure. Both are reported: `calibration_intercept` (the
        two-parameter recalibration intercept) and
...
def compare_models_repeat(y, P_by_model, reference=FUSED, seed=None):
    ...
    # BH is applied ACROSS SCENARIOS in run_model_comparisons, not here.
    # This function is called once per (scenario, source), so correcting inside
    # it would define a family of m=5 while the workbook publishes 30
    # comparisons (3 scenarios x 2 sources x 5 comparators) — six independent
    # families of five. That is anti-conservative against the set reported, and
...
# =============================================================================
# The exploratory S1/S2/S3 biomarker-group analysis is withdrawn from the
# revision and is not implemented here: the operationalisation of its four
# driver variables could not be verified against the published group sizes, and
# the treatment-by-group interaction did not survive correction for the
# cut-point search that defined the groups. The logistic solver below is
# kept because the test suite checks it against scikit-learn.
# =============================================================================
        ...
        for source, blob in (("consensus", entry["consensus"]),
            ...
            for mod in ALL_MODELS:
                ...
                for metric in ("AUROC", "AUPRC", "Brier"):
                    ...
                    # process) over the FULL range. Reducing it with `% 10000`
                    # produces 3 real collisions among this run's ~126 tags —
                    # pairs of analyses silently sharing a resample stream. Same
                    # grammar as generate_report.py, from one definition.
        ...
        for ax, sc in zip(axes[0], scs):
            ...
            if not row.empty:
                ...
                ax.text(0.03, 0.97,
                        ...
                        # risk vs observed rate". Annotating the panel with
                        # the two-parameter recalibration intercept under the
                        # bare word "intercept" would show something else
...
def run_model_comparisons(results, td, fd=None, df_perf=None):
        ...
        for source, blob in (("consensus", entry["consensus"]),
            ...
            for mod in ALL_MODELS:
                try:
                    ...
                    # ValueError here means STRUCTURAL CORRUPTION from
                    # cv_estimands.build_repeat_matrix ("patient predicted twice
                    # within repeat", or an id/prediction length mismatch) —
                    # never a benign absence, so it must not be swallowed like
                    # the KeyError above. Silently, a corrupt PKL would make the
                    # model vanish from revision_model_comparisons.xlsx with no
                    # trace, and if the vanished model were Fused_ElasticNet the
                    # whole scenario x source block would disappear while the run
                    # still exited 0. Speak up, and record it for the exit code.
    ...
    # Correcting inside compare_models_repeat would mean m=5 per call, so the
    # workbook would carry six independent families of five while presenting
    ...
    if not df.empty:
        for (sc, source), sub in df.groupby(["scenario", "source"]):
            ...
            row = {
                ...
                # The key names carry `selected_comparator` for a reason. The
                # comparator here is chosen by looking at its observed AUROC
                # (idxmax over five correlated estimates), but these p/q are
                # computed as if it had been fixed in advance. Direction
                # matters: for the claim "fused beats the best single modality"
                # the selection is CONSERVATIVE — you picked the hardest
                # comparator, so a positive significant result is safe. In the
                # other direction it is anti-conservative. The names say so.
...
def main():
    ...
    if args.data_path is not None and not Path(args.data_path).exists():
        print(f"[NO PATIENT DATA] --data_path {args.data_path} does not exist. "
              "This variant of the deposit withholds the PREDIX input matrix; "
              "see data/README.md. Every section of this script runs without "
              "it - only the exploratory survival section needs it, and that "
              "section is inert in this dataset (no follow-up columns).")
    ...
    if args.data_path and args.data_path.exists():
        ...
        # gives string columns a dedicated `str` dtype, so an `== object` check
        # would silently skip the mapping and leave 'DHP'/'T-DM1' as text.
