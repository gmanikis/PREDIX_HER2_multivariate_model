...
======================
The obvious way to summarise repeated K-fold cross-validation is to collapse
each patient's R out-of-fold predictions (one per repeat) to their MEAN and
compute AUROC / AUPRC / Brier / calibration on those n averaged values,
bootstrapping patients. This module does not do that, because the collapse
looks innocent and is not a clean estimand of a single model's performance:
  ...
  * It is severely PESSIMISTIC for models with little within-stratum signal.
    ...
    clinical model that is essentially an ER-status indicator, the collapse
    yields a patient-level AUROC of 0.41 (within each ER stratum every pCR
    patient ranks BELOW every non-pCR patient) while every fold-level estimate
    sits at 0.61-0.64 — a point estimate outside its own bootstrap interval.
    tests/test_statistics.py reproduces this artefact synthetically.
...
mean_fold_metric(folds, get_pred, metric)     -> float   (fold-averaged estimand)
...
# =============================================================================
# 4. Fold-averaged estimand (mean of the per-fold metrics), reported alongside
#    the repeat-level estimand of section 2 so the two can be compared directly
# =============================================================================
...
# settings from here, and neither keeps a copy of its own. One definition is
# what makes a single --n_boot or --seed reach every workbook: with two copies,
# one consumer can be rebound from the CLI while the other stays pinned, and the
# two workbooks then disagree about the same quantity with nothing in either to
...
def shared_seed(tag, base=DEFAULT_BOOT_SEED):
    ...
    The offset uses the FULL crc32 range rather than a narrow modulus. Folding
    it into 10,000 slots would compress the ~126 tags these scripts generate to
    a birthday expectation of ~0.8 collisions — pairs of analyses silently
    sharing one resample stream. Each CI would still be individually valid; only
    their Monte-Carlo errors would be correlated, and nothing in the output would
    say so. The full range costs nothing and keeps the streams distinct.
    """
    import zlib
...
def logistic_recalibration_rows(LP, y, max_iter=50, tol=1e-8):
    ...
    for _ in range(max_iter):
        ...
        if np.all(np.abs(da) < tol) and np.all(np.abs(db) < tol):
            break
    # A row is admitted on CONVERGENCE, not merely on staying finite. A
    # quasi-separated row can drift for all 50 damped iterations, end at
    # b = 30, pass the |b| < 50 magnitude cap and be accepted as a valid slope —
    # inflating both the mean slope and its CI. The convergence flag is carried
    # out of this function so callers can report how many repeats and bootstrap
...
def calibration_in_the_large_rows(LP, y, max_iter=50, tol=1e-8):
    ...
    log-odds on realistic data, so the two are computed and reported separately
    and the two-parameter intercept never carries this name.
...
def calibration_repeat_summary(P, y, n_boot=2000, seed=0, n_bins=10,
    ...
    out = {"n": int(n), "n_events": int(np.nansum(y)), "n_repeats": int(R),
           ...
           # calibration-in-the-large proper (the intercept with the slope held
           # at 1, not the two-parameter intercept above), and the convergence
           # bookkeeping that says how much of the requested resampling
           # contributed to the slope interval.
    ...
    for _ in range(n_boot):
        ...
        okb = np.isfinite(s_b)
        # A replicate contributes only if at least one repeat converged, and
        # within a replicate the mean is over the converged repeats only.
        # Replicates where many repeats separate — the ones carrying the most
        # extreme slopes — are therefore thinned or dropped, so this interval is
        # CONDITIONAL ON CONVERGENCE and is narrower than the unconditional one.
        # n_boot_slope_valid and n_boot_requested are reported for exactly that
        # reason: a slope resting on a small surviving fraction is then visible
        # rather than indistinguishable from one resting on all 2000.
        if okb.any():
