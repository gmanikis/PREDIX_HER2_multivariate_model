...
number of cross-validation repeats. Section 19 validates cv_estimands.py, the
...
# ------------------------------------------- 11. interaction LRT calibration
print("\n=== 11. interaction LRT ===")
# Null: no interaction -> uniform p-values
ps = []
for s in range(200):
    r = np.random.default_rng(5000+s)
    m = 180
    g = r.choice(["S1","S2","S3"], m)
    arm = r.binomial(1, .5, m).astype(float)
    er = r.binomial(1, .7, m).astype(float)
    lin = -0.3 + 0.5*(g=="S1") + 0.2*arm + 0.3*er   # main effects only
    yv = r.binomial(1, 1/(1+np.exp(-lin))).astype(float)
    d = pd.DataFrame({"group":g,"arm":arm,"Clin_ER":er,"pCR":yv})
    ps.append(RA.interaction_lrt_adjusted(d,"group","arm","pCR")["p_value"])
ps = np.array([p for p in ps if np.isfinite(p)])
check("null interaction type-I error near 5%", 0.02 <= np.mean(ps<0.05) <= 0.11,
      f"{np.mean(ps<0.05):.3f}")
check("null p-values roughly uniform", 0.40 <= np.mean(ps) <= 0.60, f"mean p={np.mean(ps):.3f}")

# True interaction -> should detect
r = np.random.default_rng(99)
m = 400
g = r.choice(["S1","S2","S3"], m)
arm = r.binomial(1, .5, m).astype(float)
er = r.binomial(1, .7, m).astype(float)
lin = -0.2 + 0.3*er + np.where(g=="S1", -1.6*arm, np.where(g=="S3", 1.6*arm, 0.0))
yv = r.binomial(1, 1/(1+np.exp(-lin))).astype(float)
d = pd.DataFrame({"group":g,"arm":arm,"Clin_ER":er,"pCR":yv})
it = RA.interaction_lrt_adjusted(d,"group","arm","pCR")
check("true interaction detected", it["p_value"] < 0.001, f"p={it['p_value']:.3e}")
check("interaction df = n_groups-1", it["df"] == 2, f"df={it['df']}")

# ---------------------------------------------------------------- 12. S groups
print("\n=== 12. assign_s_groups ===")
# Column names are read from the module default rather than hard-coded, so
# this test keeps working when the S-group operationalisation is changed via
# --s_group_spec (e.g. swapping DNA_CNV_burden for DNA_HRD).
SPEC = RA.S_GROUP_SPEC_DEFAULT
COL_ERBB2  = SPEC["erbb2_scna"]
COL_INSTAB = SPEC["genome_instability"]
COL_ADC    = SPEC["adc_trafficking"]
COL_IMM    = SPEC["type2_immunity"]

m = 181
dfS = pd.DataFrame({
    "patient_id": np.arange(m),
    COL_ERBB2:  rng.normal(2,1,m),
    COL_INSTAB: rng.normal(0,1,m),
    COL_ADC:    rng.normal(0,1,m),
    COL_IMM:    rng.normal(0,1,m),
    "Clin_Arm": rng.binomial(1,.5,m).astype(float),
    "Clin_ER": rng.binomial(1,.7,m).astype(float),
    "pCR": rng.binomial(1,.45,m).astype(float),
})
lab, cut = RA.assign_s_groups(dfS, q=0.5)
check("spec resolves to 4 distinct columns", len({COL_ERBB2,COL_INSTAB,COL_ADC,COL_IMM})==4)
check("all patients labelled", lab.notna().all() and len(lab)==m)
check("labels are S1/S2/S3", set(lab.unique()) <= {"S1","S2","S3","Unclassifiable"}, str(set(lab.unique())))
check("partition is exhaustive & disjoint", lab.value_counts().sum()==m)
# S1 must satisfy both node-1 conditions
s1_mask = lab=="S1"
check("S1 obeys node 1",
      bool((dfS.loc[s1_mask,COL_ERBB2] >= cut["erbb2_scna"]["threshold"]).all()
       and (dfS.loc[s1_mask,COL_INSTAB] >= cut["genome_instability"]["threshold"]).all()))
s3_mask = lab=="S3"
check("S3 obeys node 2 and excludes S1",
      bool((dfS.loc[s3_mask,COL_ADC] >= cut["adc_trafficking"]["threshold"]).all()
       and (dfS.loc[s3_mask,COL_IMM] <= cut["type2_immunity"]["threshold"]).all()
       and not (s1_mask & s3_mask).any()))
# Missing driver -> Unclassifiable
dfS2 = dfS.copy(); dfS2.loc[0,COL_ERBB2] = np.nan
lab2,_ = RA.assign_s_groups(dfS2, q=0.5)
check("missing driver -> Unclassifiable", lab2.iloc[0]=="Unclassifiable", lab2.iloc[0])
# Missing columns handled
lab3, info = RA.assign_s_groups(dfS.drop(columns=[COL_IMM]))
check("missing column reported not crashed", lab3 is None and "missing" in info)

# ---------------------------------------------------------------- 13. sweep
print("\n=== 13. cut-point sweep ===")
sw, _ = RA.sweep_cutpoints(dfS, "Clin_Arm", "pCR")
check("sweep covers 13 cutpoints", len(sw)==13, f"{len(sw)}")
check("sweep group sizes sum correctly",
      bool(((sw["n_S1"]+sw["n_S2"]+sw["n_S3"])==sw["n_classified"]).all()))
check("sweep p-values in [0,1]",
      bool(sw["interaction_p"].dropna().between(0,1).all()))

print("\n=== 14. permutation p (small n_perm) ===")
pm = RA.cutpoint_permutation_p(dfS,"Clin_Arm","pCR",n_perm=40)
check("permutation p in (0,1]", 0 < pm["permutation_p"] <= 1, f"{pm['permutation_p']:.4f}")
check("perm p >= observed min p", pm["permutation_p"] >= pm["observed_min_p"],
      f"perm {pm['permutation_p']:.4f} vs min {pm['observed_min_p']:.4f}")
check("13 cutpoints searched recorded", pm["n_cutpoints_searched"]==13)

# ---------------------------------------------------------------- 15. survival
print("\n=== 15. Kaplan-Meier / log-rank / Cox ===")
...
print("\n=== 19a. cv_estimands: per-repeat metrics == scikit-learn ===")
...
print("\n=== 19b. vectorised logistic recalibration == scikit-learn ===")
n_cal2 = 1500
...
print("\n=== 19d. the held-out-outcome artefact is reproduced (and avoided) ===")
...
print("\n=== 19e. cluster bootstrap CI vs naive row-level CI ===")
R_e, n_e = 4, 80
...
print("\n=== 19f. paired cluster-bootstrap delta ===")
...
print("\n=== 19h. fold operating points with cluster CI ===")
...
print("\n=== 19i. calibration per repeat + pooled reliability ===")
