...
**110 metrics in total**, counted from the analysis file itself rather than
typed. The complete list is in `supplementary/S-ML8_candidate_panel.xlsx`
(sheet `Candidate_panel`).
...
Three reductions are applied, and none uses the outcome across the cohort.
...
all three counts are read from the analysis file and from `TIER1_REMOVE` itself,
not typed. Correlations are recomputed on the complete-case cohort (n = 110),
with categorical columns encoded 0/1 first; the pCR label is not used anywhere
in this step.
...
This is not a second route by which information could enter, and it is
deliberately not a per-fold filter. A correlation filter fitted inside each fold
would keep whichever member of a redundant pair won *that fold's* univariate
contest against pCR, so the surviving representative would rotate from fold to
fold — the rotating-basis instability that makes penalised-regression signatures
uninterpretable, and that shows up as a gene and its identical OncoKB twin each
accumulating part of one selection frequency. No such filter runs here
(`CORR_FILTER_MODS` is empty). The fixed list is instead long enough to leave no
...
### 2. A per-scenario, within-modality deduplication, also before any split

Collinearity is a property of a population: a pair that is separable in the
pooled cohort can be identical within one arm. So a second deduplication runs on
each scenario's own complete-case cohort — the pooled cohort and each treatment
arm — before that scenario is split (`--dedup_per_scenario`, implemented in
`code/feature_deduplication.py`). Pairs are compared **within modality only**,
because prediction-level fusion never puts two modalities in one design matrix,
and by the statistic the data type warrants:

| Data type | Statistic | Threshold | Why not correlation |
|---|---|---:|---|
| continuous | \|Pearson r\| | 0.80 | — |
| segment-called copy number (`_CNA`, > 10 distinct values) | exact agreement | 0.80 | genes on one called segment receive one value; on the capped scale the discordant minority drags r down while the columns are identical in most patients (`DNA_ERBB2_CNA` and `DNA_RPL19_CNA`: identical in 86 % of the pooled cohort) |
| binary indicator | Cohen's κ | 0.80 | raw agreement is inflated by rarity: two 5 %-prevalence mutations agree in ~90 % of rows without ever co-occurring |

One representative per cluster is kept by a rule fixed in advance — a named
driver over its co-amplified passengers (`DNA_ERBB2_CNA` over `DNA_MED1_CNA` and
`DNA_RPL19_CNA`), the plain mutation call over its `_pathway` / `_oncokb`
aggregate, otherwise the more completely measured member, then alphabetical —
and the pCR label is never read. The step removes **11 (pooled),
12 (DHP) and 6 (T-DM1)** candidates, so **81, 80 and
86 candidates enter the fold loop** in the three scenarios. Every removal,
the feature retained in its place and the statistic that decided it are in
`results/<scenario>/<scenario>_deduplication_audit.csv`. Like the fixed list it
can be switched off (omit `--dedup_per_scenario`) so that its effect can be
measured.

### 3. The univariate association step, moved inside every training fold
...
Of the 81 candidates entering each outer fold of the pooled-cohort
analysis, the median number surviving preprocessing and the in-fold screen is
10 transcriptomic, 5 genomic and 12 proteomic features
(clinical 5 and WSI 3, unscreened), from which the
events-per-variable cap admits a median signature of 5 transcriptomic,
4 genomic, 5 proteomic, 5 clinical and 3 WSI features.
(Medians over the pooled outer folds, read from
`results/global/global_elasticnet_results.pkl`.)

Every count on this page is read from the analysis file, from `TIER1_REMOVE`,
from the deposited deduplication audits and from the deposited fold records at
build time, never typed, so the documentation cannot drift away from the code.
