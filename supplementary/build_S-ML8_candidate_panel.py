...
----------------------------------
The table is **S-ML8**; the file is a build tool and its name is not a citation
anyone reads. `build_github_repo.py` deposits it as
`supplementary/build_S-ML8_candidate_panel.py`. What it WRITES carries the
number the manuscript and the response letter cite:
`supp_table_S-ML8_candidate_panel.xlsx`, sheets headed S-ML8a and S-ML8b.

WHY THE REMOVAL LIST IS NOT TYPED INTO THIS FILE
------------------------------------------------
A hand-copied duplicate of TIER1_REMOVE in this file would go stale silently:
nothing would fail, and the script would simply emit a supplementary table that
no longer described the pipeline. The list is therefore PARSED from the
pipeline source, which is the one authoritative definition, and every count in
the table is derived from that parse plus the data file.
...
The parser understands two forms: a plain list literal, and a literal followed
by a guarded mutation such as `if not KEEP_X: TIER1_REMOVE.append("X")`.
Anything it does not recognise is a hard failure, never a silent partial
answer.
...
# Every path is resolved from THIS FILE'S OWN LOCATION, never from an absolute
# path, so the script runs wherever the repository is checked out. Two layouts
# are supported and both are probed:
#   working tree — this script sits one level below the root, the input matrix
#                  is at the root, and the workbook belongs beside this script;
#   deposit      — this script ships in supplementary/, beside code/ and data/,
#                  and writes the workbook beside itself under its own name, so
#                  it can never overwrite the deposited S-ML8 workbook it would
#                  be compared against.
_HERE = Path(__file__).resolve().parent
ROOT = _HERE.parent

DATA_NAME = "clin_multiomics_curated_metrics_PREDIX_HER2_new.txt"
DATA_CANDIDATES = [
    ROOT / DATA_NAME,
    ROOT / "data" / DATA_NAME,
    _HERE / DATA_NAME,
]
DATA = next((p for p in DATA_CANDIDATES if p.is_file()), DATA_CANDIDATES[0])
OUT = _HERE / "supp_table_S-ML8_candidate_panel.xlsx"

# Where the authoritative TIER1_REMOVE lives. First existing path wins; the
# later entries cover the deposited layout, where this script ships in
# supplementary/ next to code/.
PIPELINE_CANDIDATES = [
    ROOT / "multimodal_pcr_pipeline.py",
    ROOT / "code" / "multimodal_pcr_pipeline.py",
...
def parse_tier1_remove(path):
    ...
    # utf-8-sig, not utf-8: a copy of the pipeline that has picked up a BOM
    # would otherwise abort ast.parse with "invalid non-printable character
    # U+FEFF".
    ...
    # Mutations after the literal, e.g. a guarded append behind a module flag.
...
# check matters: RNA_TILs is itself removed, while the code comments beside
# RNA_T-cells and RNA_CD45 name it as their counterpart. The pipeline's own
# comment block settles the substitution — "KEEPING mRNA-CD8A
...
if len(TIER1) != EXPECT_TIER1_LISTED:
    fail(f"TIER1_REMOVE in {PIPE} lists {len(TIER1)} features; this script "
         f"expects {EXPECT_TIER1_LISTED}.\n"
         f"parsed: {TIER1}\n"
         ...
         f"confirm that the results directory was produced by this same "
         f"pipeline source, and re-run.")
...
if not DATA.is_file():
    ...
    fail(
        ...
        "==============================================================================\n"
        "build_S-ML8_candidate_panel.py\n"
...
if sha != EXPECT_DATA_SHA256:
    fail(f"the input file is not the one this table was verified against.\n"
         ...
         f"Confirm that this is the input matrix the reported results were "
         f"produced from, update EXPECT_DATA_SHA256 and the other tripwires, "
         f"and re-run.")
...
if bad:
    fail("the table would not describe the analysis it is meant to describe:\n"
         ...
         + f"\nThese are tripwires, not inputs — the computed value is what the "
           f"data and the pipeline actually say. Reconcile the input matrix and "
           f"the pipeline source with the reported analysis, update the EXPECT_* "
           f"block, and re-run.")
